...
try:
    ...
    _SYCL_SO = _OP_DIR / "reduce_min_sycl.so"

    @ProviderRegistry.register_vendor_impl("reduce_min", "sycl_ext")
    class SyclExtReduceMinOp(ReduceMinOp):
        _SO_MODULE = None
        ...
        def __init__(self, args_dict, backend, *args, **kwargs):
            ...
            if self.arg_type != "default":
                raise ValueError("SyclExtReduceMinOp only supports arg_type=default")
        ...
        def _load_sycl_so(cls):
            ...
            if not _SYCL_SO.is_file():
                raise FileNotFoundError(
                    f"reduce_min sycl extension not found: {_SYCL_SO}. "
                    "Please run backends/INTEL/ops/sycl_ext/build.sh first."
                )

            spec = importlib.util.spec_from_file_location("reduce_min_sycl", str(_SYCL_SO))
        ...
        def _resolve_rows_per_group(self, value):
            ...
            try:
                ...
                raise ValueError(
                    ...
                    "SYCL_EXT_REDUCE_MIN_ROWS_PER_GROUP: "
                    f"{value}"
                ) from e
        ...
        def _parse_so_config(self):
            ...
            iterations = self._get_int_option(
                ...
                env_key="SYCL_EXT_REDUCE_MIN_ITERATIONS",
                default=100,
            )
            verify = self._get_int_option(
                key="sycl_ext_verify",
                env_key="SYCL_EXT_REDUCE_MIN_VERIFY",
                default=0,
            )
            warmup = self.args_dict.get(
                "sycl_ext_warmup",
                os.getenv("SYCL_EXT_REDUCE_MIN_WARMUP", 0),
            )
            try:
                warmup = int(warmup)
            except (TypeError, ValueError) as e:
                raise ValueError(
                    f"Invalid integer option for sycl_ext_warmup/SYCL_EXT_REDUCE_MIN_WARMUP: {warmup}"
                ) from e

            input_mode = self.args_dict.get(
                "sycl_ext_input_mode",
                os.getenv("SYCL_EXT_REDUCE_MIN_INPUT_MODE", "sin"),
            )
            input_mode = str(input_mode).strip().lower()
            if input_mode not in {"sin", "tie", "nan_head", "nan_mix"}:
                raise ValueError(
                    "Invalid sycl_ext_input_mode/SYCL_EXT_REDUCE_MIN_INPUT_MODE. "
                    "Expected one of: sin, tie, nan_head, nan_mix"
                )

            smalldim_mode = self.args_dict.get(
                "sycl_ext_smalldim_mode",
                os.getenv("SYCL_EXT_REDUCE_MIN_SMALLDIM_MODE", "multirow"),
            )
            smalldim_mode = str(smalldim_mode).strip().lower()
            if smalldim_mode not in {"baseline", "multirow"}:
                raise ValueError(
                    "Invalid sycl_ext_smalldim_mode/SYCL_EXT_REDUCE_MIN_SMALLDIM_MODE. "
                    "Expected one of: baseline, multirow"
                )

            rows_per_group = self.args_dict.get(
                "sycl_ext_rows_per_group",
                os.getenv("SYCL_EXT_REDUCE_MIN_ROWS_PER_GROUP", "auto"),
            )
            ...
            return {
                ...
                "so_variant": self.args_dict.get(
                    "sycl_ext_so_variant",
                    os.getenv("SYCL_EXT_REDUCE_MIN_SO_VARIANT", "so_v0"),
                ),
            }
        ...
        def _run_sycl_so_compute(self, tensor_mapping):
            ...
            self._sycl_so.reduce_min_compute_into(
                tensor_mapping["src"],
                tensor_mapping["min_value"],
                tensor_mapping["min_indices"],
    ...
    print(f"[SyclExtReduceMinOp] Failed to register: {e}")
    pass
