 ...
 *
 * Execution outline:
 *   1. For each token row, compute SwiGLU(x1, x2) and apply smooth_scale.
 *   2. Track the row-wise max absolute value needed for dynamic quantization.
 *   3. Store the intermediate float row in SLM so the second pass can quantize
 *      without rereading the source tensors.
 *   4. Write quantized outputs plus the per-token scale.
 */
...
#pragma unroll
    ...
    // Encode the SLM size in the kernel type so each hidden-size bucket gets a
    // fixed allocation known at compile time.
    ...
    // Prefer the largest clean unroll factor that still leaves enough parallel
    // blocks to occupy the work-group.
    for (int u : {8, 4, 2}) {
