...
# xccl helpers are loaded lazily from vendor_ops at runtime
_xccl_loaded = False
run_perf = None
run_infer_loop = None

def _load_xccl():
    global _xccl_loaded, run_perf, run_infer_loop
    if _xccl_loaded:
        return
    try:
        from xpu_perf_provider_xccl.perf import run_perf as _rp
        from xpu_perf_provider_xccl.infer_loop import run_infer_loop as _ril
        run_perf = _rp
        run_infer_loop = _ril
    except ImportError:
        pass
