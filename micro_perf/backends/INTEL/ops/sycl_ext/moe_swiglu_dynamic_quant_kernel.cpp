...
void moe_swiglu_dynamic_quant_impl(
    ...
    constexpr uint32_t REDUCTION_SLM_BYTES = 1056;
    ...
    auto launch_swiglu = [&](auto unroll_tag) {
        ...
        queue.submit([&](sycl::handler& cgh) {
            cgh.parallel_for(sycl::nd_range<2>(GlobalRange, LocalRange), [=](sycl::nd_item<2> item) SYCL_ESIMD_KERNEL [[intel::kernel_args_restrict]] {
                // Keep only reduction scratch in SLM so work-group residency is not capped by hidden_size.
                slm_init(REDUCTION_SLM_BYTES);
                ...
                // Pass 1: Read, compute, and track extrema.
...
#pragma unroll
                ...
                // Pass 2: Recompute with compact SLM scratch to preserve occupancy.
                for (int bid = loc_id; bid < num_blocks; bid += wg_size) {
#pragma unroll
                    for (int u = 0; u < UNROLL; ++u) {
                        simd<T_in, CHUNK> x1 = block_load<T_in, CHUNK>(
                        ...
                        if constexpr (std::is_same_v<T_out, int8_t>) {
                            ...
                            quantized_out = fast_cvt_float_to_e4m3fn<CHUNK>(scaled_swiglu_tokens * recip_scale);
                        }
